...
public class CorreoGenerate {
	...
	public static String generarCorreo(String correo, int countDuplicateEmail, boolean isUpdate) {
		if(isUpdate == false) {
			int arrobaIndex = correo.indexOf('@');
			return correo.substring(0, arrobaIndex) + "." + (countDuplicateEmail + 1) + correo.substring(arrobaIndex);
		}else {
			int arrobaIndex = correo.indexOf('@');
			return correo.substring(0, arrobaIndex) + "." + (countDuplicateEmail) + correo.substring(arrobaIndex);
		}
	}
}
